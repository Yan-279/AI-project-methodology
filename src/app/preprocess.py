...
def preprocess(dataframe: pd.DataFrame, MODEL_DIR : Path) -> pd.DataFrame:
    processed_df = dataframe.copy()
    processed_df = preprocess_continuous_data(processed_df)
    processed_df = preprocess_categorical_data(processed_df, MODEL_DIR)
    return processed_df
...
def preprocess_categorical_data(dataframe: pd.DataFrame, MODEL_DIR : Path) -> pd.DataFrame:
    dataframe = impute_categorical_missing_values(dataframe)
    encoder = get_one_hot_encoder(dataframe, MODEL_DIR)
...
def get_one_hot_encoder(dataframe: pd.DataFrame, MODEL_DIR : Path) -> OneHotEncoder:
